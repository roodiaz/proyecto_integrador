...
/// para construir las tarjetas, la distribución de cartera, las notificaciones recientes, el gráfico de desempeño y las últimas operaciones del dashboard.
/// </summary>
public class DashboardServiceTests
{
    ...
    private DashboardService CreateService() => new(_userRepository.Object, _portfolioRepository.Object, _portfolioHistoryRepository.Object, _marketPriceCacheService.Object, _logger.Object, _alertRepository.Object, _priceHistoryRepository.Object, _transactionRepository.Object, _notificationRepository.Object);
    ...
    private static PortfolioHistory History(decimal totalValue, DateTime date) => new() { UserId = 1, Date = date, TotalValue = totalValue };
    ...
    /// <summary>Verifica que, si el usuario no existe, se devuelva una respuesta de error sin consultar la cartera.</summary>
    [Fact]
    public async Task GetTopCardsAsync_WhenUserDoesNotExist_ShouldReturnErrorResponse()
    {
        _userRepository.Setup(r => r.GetByIdAsync(It.IsAny<int>())).ReturnsAsync((User?)null);

        var result = await CreateService().GetTopCardsAsync(1);

        Assert.False(result.Success);
        Assert.Equal("Usuario no encontrado", result.Message);
        _portfolioRepository.Verify(r => r.GetByUserAsync(It.IsAny<int>()), Times.Never);
    }
    ...
    public async Task GetTopCardsAsync_WhenDataIsValid_ShouldReturnComputedTopCards()
    {
        ...
        _userRepository.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(UserEntity(balance: 1000));
    ...
    {
        var asset = AssetEntity();
        var portfolio = new List<Portfolio> { PortfolioEntity(asset, quantity: 5) };

        _userRepository.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(UserEntity(balance: 500));
        _portfolioRepository.Setup(r => r.GetByUserAsync(1)).ReturnsAsync(portfolio);
    ...
    public async Task GetPortfolioDistributionAsync_WhenDataIsValid_ShouldReturnDistributionGroupedBySector()
    {
        ...
        {
            PortfolioEntity(techAsset, quantity: 10),
            new() { Id = 2, UserId = 1, AssetId = energyAsset.Id, Asset = energyAsset, Quantity = 5, AvgPrice = 50 }
        };

        _portfolioRepository.Setup(r => r.GetByUserAsync(1)).ReturnsAsync(portfolio);
        _marketPriceCacheService.Setup(s => s.GetPricesAsync(It.IsAny<List<string>>())).ReturnsAsync(Prices(("AAPL", 100), ("XOM", 50)));

        var result = await CreateService().GetPortfolioDistributionAsync(1);
    ...
    public async Task GetPortfolioDistributionAsync_WhenPortfolioIsEmpty_ShouldReturnEmptyList()
    {
        ...
        _marketPriceCacheService.Setup(s => s.GetPricesAsync(It.IsAny<List<string>>())).ReturnsAsync(new MarketPricesResponseDto());
    ...
    {
        _portfolioRepository.Setup(r => r.GetByUserAsync(It.IsAny<int>())).ThrowsAsync(new Exception("db error"));
    ...
    /// <summary>Verifica que, si el usuario no existe, se devuelva una respuesta de error sin consultar el historial.</summary>
    [Fact]
    public async Task GetPerformanceChartAsync_WhenUserDoesNotExist_ShouldReturnErrorResponse()
    {
        _userRepository.Setup(r => r.GetByIdAsync(It.IsAny<int>())).ReturnsAsync((User?)null);

        var result = await CreateService().GetPerformanceChartAsync(1, new DashboardPerformanceChartFilterDto { Period = "1M" });

        Assert.False(result.Success);
        Assert.Equal("Usuario no encontrado", result.Message);
        _portfolioHistoryRepository.Verify(r => r.GetByUserAndDateAsync(It.IsAny<int>(), It.IsAny<DateTime>()), Times.Never);
    }
    ...
    public async Task GetPerformanceChartAsync_WhenHistoryDataIsInsufficient_ShouldReturnEmptyChart()
    {
        ...
        _portfolioHistoryRepository.Setup(r => r.GetByUserAndDateAsync(It.IsAny<int>(), It.IsAny<DateTime>())).ReturnsAsync(new List<PortfolioHistory>());
    ...
    public async Task GetPerformanceChartAsync_WhenDataIsValidWithDailyPeriod_ShouldReturnChartWithComparisonData()
    {
        ...
        _userRepository.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(UserEntity(balance: 500));
        _portfolioHistoryRepository.Setup(r => r.GetByUserAndDateAsync(It.IsAny<int>(), It.IsAny<DateTime>())).ReturnsAsync(new List<PortfolioHistory> { History(1000, date1), History(1100, date2) });
    ...
    public async Task GetPerformanceChartAsync_WhenRepositoryThrows_ShouldReturnErrorResponse()
    {
        ...
        _portfolioHistoryRepository.Setup(r => r.GetByUserAndDateAsync(It.IsAny<int>(), It.IsAny<DateTime>())).ThrowsAsync(new Exception("db error"));
    ...
    public async Task GetLatestTransactionsAsync_WhenTransactionsExist_ShouldReturnMappedList()
    {
        ...
        {
            new() { Id = 1, UserId = 1, AssetId = asset.Id, Asset = asset, Type = TransactionType.Buy, Quantity = 5, Price = 100, Total = 500.456m, CreatedAt = DateTime.UtcNow },
            new() { Id = 2, UserId = 1, AssetId = asset.Id, Asset = asset, Type = TransactionType.Sell, Quantity = 2, Price = 110, Total = 220, CreatedAt = DateTime.UtcNow }
        };

        _transactionRepository.Setup(r => r.GetLatestByUserAsync(1, 5)).ReturnsAsync(transactions);

        var result = await CreateService().GetLatestTransactionsAsync(1);
    ...
    /// <summary>Verifica que, cuando el usuario no tiene operaciones registradas, se devuelva una lista vacía.</summary>
    [Fact]
    public async Task GetLatestTransactionsAsync_WhenUserHasNoTransactions_ShouldReturnEmptyList()
    {
        _transactionRepository.Setup(r => r.GetLatestByUserAsync(1, 5)).ReturnsAsync(new List<Transaction>());
    ...
    {
        _transactionRepository.Setup(r => r.GetLatestByUserAsync(It.IsAny<int>(), It.IsAny<int>())).ThrowsAsync(new Exception("db error"));
